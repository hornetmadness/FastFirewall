...
`--all-packages` is required — without it, transitive dependencies of workspace member plugins (e.g. `pyroute2` from the networking plugin) are not installed and the dev environment is incomplete.
