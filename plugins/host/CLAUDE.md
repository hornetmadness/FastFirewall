...
| `initsys.service.start` | `{service_name}` | Ensures an *existing* service (not one FF registers itself — see `initsys.service.add` for that) is both running and enabled for boot, via `_enable_service`. Skips silently only when already running **and** enabled. Re-checks status after applying and reports failure if the service did not end up running. | `{"success": bool}` |
